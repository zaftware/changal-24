...
const ins = db.prepare(`INSERT OR IGNORE INTO posts(source,title,url,body,published_at,hash,score) VALUES (?,?,?,?,?,?,?)`);
let n=0;
for(const it of items){
  const h = hashOf((it.url||'')+'|'+(it.title||''));
  const score = Math.min(100, (it.body?.length||0)/30);
  const info = ins.run(it.source,it.title,it.url,it.body,it.publishedAt,h,score);
  n += info.changes;
}
